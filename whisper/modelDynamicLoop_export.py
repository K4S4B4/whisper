...
class TextDecoder_dynamicLoop_tiny_body(TextDecoder_dynamicLoop):
    def forward(self, trip_count, in_cond, in_tokens, in_positions, 
        ...
        out_cond, out_token, out_positions, out_self_kv_list, out_token_scan = self.loopBody(trip_count, in_cond, in_tokens, in_positions, self_kv_list, n_layer_cross_k, n_layer_cross_v)
...
def export_TextDecoder_dynamicLoop_driver(name, model, n_ctx_in: int, n_ctx_out: int, n_audioFeature: int):
    ...
    output_names = ['out_token_scan', 'trip_count', 'in_cond', 'in_tokens_dum', 'in_positions_dum']
    ...
    dynamic_axes = dict()
    dynamic_axes['out_token_scan'] = {0: 'n_tokens_out'}
    if n_ctx_in < 0:
    ...
    for i in range(n_ctx_in):
        token_list.append(torch.tensor(i, dtype=torch.int32).to(device))
...
def export_TextDecoder_dynamicLoop_body(name, model, n_ctx_in: int, n_ctx_out: int, n_audioFeature: int):
    ...
    input_names = ['trip_count', 'in_cond', 'in_tokens', 'in_positions']
    for i in range(n_layer):
    ...
    output_names = ['out_cond', 'out_tokens', 'out_positions']
    for i in range(n_layer):
    ...
    inputsList = [trip_count, in_cond, dummy_tokens, dummy_positions]
